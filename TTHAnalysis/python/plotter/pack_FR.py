...
if __name__ == "__main__":
    ...
    if True:
       ...
       ta_data = ["tauFR_numLL_vs_pt_pfmt_fix_data_sub_prefit", "tauFR_numLL_vs_pt_pfmt_fix_data_sub_prefit"]
       ta_data_jet = ["tauFR_numLL_vs_pt_jet_pfmt_fix_data_sub_prefit", "tauFR_numLL_vs_pt_jet_pfmt_fix_data_sub_prefit"]
       #XsD    = [ "DY",  "data_comb" ]
       ...
       Plots="/eos/user/j/jheikkil/www/FR1412/"
       Z3l=None # "z3l/v2.1.1"
       #QCD="qcd1l/v3.0.1"
       QCD="qcd1l/v3.1"
       ...
       readMany2D(ta, h2d_tau, "/".join([Plots, "FR_tau_%s.root"]), "%s", etaslices_tau, (5,999) )
       readMany2D(ta_jet, h2d_tauJET, "/".join([Plots, "FR_tau_%s.root"]), "%s", etaslices_tau, (5,999) )
       ##readMany2D([ make2D(outfile, "FR_tau", ptbins_tau, etabins_tau)]mu_data, h2d_muD, "/".join([Plots, "FR_muon_%s.root"]), "%s", etaslices_mu, (5,999) )
       readMany2D(ta_data, h2d_tauD, "/".join([Plots, "FR_tau_%s.root"]), "%s", etaslices_tau, (5,999) )
       readMany2D(ta_data_jet, h2d_tauDJET, "/".join([Plots, "FR_tau_%s.root"]), "%s", etaslices_tau, (5,999) )
       ...
       for h in h2d_tau + h2d_tauJET + h2d_tauDJET + h2d_tauD: outfile.WriteTObject(h)
